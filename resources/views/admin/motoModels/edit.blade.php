        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.motoModel.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.moto-models.update", [$motoModel->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('model') ? 'has-error' : '' }}">
                            <label class="required" for="model">{{ trans('cruds.motoModel.fields.model') }}</label>
                            <input class="form-control" type="text" name="model" id="model" value="{{ old('model', $motoModel->model) }}" required>
                            @if($errors->has('model'))
                        ...
                        <div class="form-group {{ $errors->has('moto_brand') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="moto_brand_id" id="moto_brand_id" required>
                                @foreach($moto_brands as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('moto_brand_id') ? old('moto_brand_id') : $motoModel->moto_brand->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
