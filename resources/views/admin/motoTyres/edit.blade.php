        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.motoTyre.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.moto-tyres.update", [$motoTyre->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                            <label for="title">{{ trans('cruds.motoTyre.fields.title') }}</label>
                            <input class="form-control" type="text" name="title" id="title" value="{{ old('title', $motoTyre->title) }}">
                            @if($errors->has('title'))
                        ...
                        <div class="form-group {{ $errors->has('tagline') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="tagline" id="tagline" value="{{ old('tagline', $motoTyre->tagline) }}">
                        ...
                        <div class="form-group {{ $errors->has('moto_brand') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="moto_brand_id" id="moto_brand_id" required>
                                @foreach($moto_brands as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('moto_brand_id') ? old('moto_brand_id') : $motoTyre->moto_brand->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('moto_model') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="moto_model_id" id="moto_model_id" required>
                                @foreach($moto_models as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('moto_model_id') ? old('moto_model_id') : $motoTyre->moto_model->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('moto_engine') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="moto_engine_id" id="moto_engine_id" required>
                                @foreach($moto_engines as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('moto_engine_id') ? old('moto_engine_id') : $motoTyre->moto_engine->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('moto_width') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="moto_width_id" id="moto_width_id">
                                @foreach($moto_widths as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('moto_width_id') ? old('moto_width_id') : $motoTyre->moto_width->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('moto_size') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="moto_size_id" id="moto_size_id">
                                @foreach($moto_sizes as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('moto_size_id') ? old('moto_size_id') : $motoTyre->moto_size->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('moto_ratio') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="moto_ratio_id" id="moto_ratio_id">
                                @foreach($moto_ratios as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('moto_ratio_id') ? old('moto_ratio_id') : $motoTyre->moto_ratio->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('categories') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="categories[]" id="categories" multiple>
                                @foreach($categories as $id => $category)
                                    <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || $motoTyre->categories->contains($id)) ? 'selected' : '' }}>{{ $category }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('short_description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="short_description" id="short_description">{!! old('short_description', $motoTyre->short_description) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('long_description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="long_description" id="long_description">{!! old('long_description', $motoTyre->long_description) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('features') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="features" id="features">{!! old('features', $motoTyre->features) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('specifications') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="specifications" id="specifications">{!! old('specifications', $motoTyre->specifications) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('advantages') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="advantages" id="advantages">{!! old('advantages', $motoTyre->advantages) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('warranty') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="warranty" id="warranty">{!! old('warranty', $motoTyre->warranty) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('video') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="video" id="video">{{ old('video', $motoTyre->video) }}</textarea>
                            @if($errors->has('video'))
