                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.faqs.update", [$faq->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('faq_title') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="faq_title" id="faq_title" value="{{ old('faq_title', $faq->faq_title) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('faq_content') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="faq_content" id="faq_content">{!! old('faq_content', $faq->faq_content) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('faq_category') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="faq_category_id" id="faq_category_id" required>
                                @foreach($faq_categories as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('faq_category_id') ? old('faq_category_id') : $faq->faq_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
