        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.warrantyClaim.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.warranty-claims.update", [$warrantyClaim->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('invoice_number') ? 'has-error' : '' }}">
                            <label class="required" for="invoice_number">{{ trans('cruds.warrantyClaim.fields.invoice_number') }}</label>
                            <input class="form-control" type="text" name="invoice_number" id="invoice_number" value="{{ old('invoice_number', $warrantyClaim->invoice_number) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('product_name') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="product_name_id" id="product_name_id" required>
                                @foreach($product_names as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('product_name_id') ? old('product_name_id') : $warrantyClaim->product_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('product_size') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="product_size_id" id="product_size_id" required>
                                @foreach($product_sizes as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('product_size_id') ? old('product_size_id') : $warrantyClaim->product_size->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('retailer') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="retailer_id" id="retailer_id">
                                @foreach($retailers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('retailer_id') ? old('retailer_id') : $warrantyClaim->retailer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
