        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.modelCombination.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.model-combinations.update", [$modelCombination->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('brand') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="brand_id" id="brand_id" required>
                                @foreach($brands as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('brand_id') ? old('brand_id') : $modelCombination->brand->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('brand'))
                        ...
                        <div class="form-group {{ $errors->has('car_model') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="car_model_id" id="car_model_id">
                                @foreach($car_models as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('car_model_id') ? old('car_model_id') : $modelCombination->car_model->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('years') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="years[]" id="years" multiple required>
                                @foreach($years as $id => $year)
                                    <option value="{{ $id }}" {{ (in_array($id, old('years', [])) || $modelCombination->years->contains($id)) ? 'selected' : '' }}>{{ $year }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('years'))
                        ...
                        <div class="form-group {{ $errors->has('engine') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="engine_id" id="engine_id" required>
                                @foreach($engines as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('engine_id') ? old('engine_id') : $modelCombination->engine->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
